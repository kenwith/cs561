...
class SRServerListener(EventMixin):
  ...
  def __init__ (self, address=('127.0.0.1', 8888)):
    port = address[1]
    self.listenTo(core.cs561_ofhandler)
    self.srclients = []
  ...
  def _handle_packet_msg(self, conn, vns_msg):
    ...
    core.cs561_srhandler.raiseEvent(SRPacketOut(pkt, out_port))
...
class cs561_srhandler(EventMixin):
...
def launch (transparent=False):
  ...
  """
  core.registerNew(cs561_srhandler)
